...
#[Consumer(exchange: 'task_worker_cancel', routingKey: 'task_worker_cancel', queue: 'task_worker_cancel_queue', name: "TaskWorkerCancelConsumer", nums: 1)]
class TaskWorkerCancelConsumer extends ConsumerMessage
{
    #[Inject]
    ...
    public function consumeMessage($data, AMQPMessage $message): Result
    {
        ...
        try {
            ...
            // 获取任务统计生产者
            $task_worker_stats_producer = new TaskWorkerStatsProducer(['member_id' => $member_id, 'order_no' => $order_no, 'task_type' => $payload['type'], 'status' => 'cancel', 'amount' => 0]);
            // 发送队列
            ...
            $data['retry']++;
            // 获取生产者
            $task_worker_cancel_producer = new TaskWorkerCancelProducer($data);
            // 发送到队列
            $this->producer->produce($task_worker_cancel_producer);
            // 完成消费
            return Result::ACK;
        }
    }
